...
%}
% [trainsample.i,valsample.i,testsample.i] =dividerand(input,0.7,0.15,0.15);
%  
% [trainsample.o,valsample.o,testsample.o] =dividerand(output,0.7,0.15,0.15);


%{
创建神经网络
使用newff函数
改进：将参数提取出来设置为变量http://blog.sina.com.cn/s/blog_5f853eb10100zyib.html
%}
...
% [normvalidateoutput,validatePerf]=sim(net,valsample.i,[],[],valsample.o);%验证的数据，经BP得到的结果
...
将获得预测结果output
%}
% trainOutput = mapminmax('reverse',normTrainOutput,ts);%训练数据：BP得到的归一化后的结果output
% trainInsect = mapminmax('reverse',trainSamples.o,ts);%训练数据已知的输出output
% validateOutput = mapminmax('reverse',normValidateOutput,ts);%变量数据：BP得到的归一化的结果output
% validateInsect = mapminmax('reverse',validateSamples.o,ts);%变量的数据已知的输出output
% testOutput = mapminmax('reverse',normTestOutput,ts);%测试数据：BP得到的归一化的结果output
% testInsect = mapminmax('reverse',testSamples.o,ts);%测试数据已知的输出output
